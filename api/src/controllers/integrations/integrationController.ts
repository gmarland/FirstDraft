...
export class IntegrationController {
  ...
  public readonly getJiraIntegration: RequestHandler = async (req, res, next) => {
    try {
      const user = currentUser(req);
      res.json(await this.jiraIntegrations.getSettingsForUser(user.userId, routeParam(req.params, "integrationId")));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly updateJiraConnection: RequestHandler = async (req, res, next) => {
    try {
      ...
      const validationError = validateConnectionInput(input);
      if (validationError) return res.status(400).json({ error: validationError });
      const existing = await this.jiraIntegrations.getSettingsForUser(user.userId, routeParam(req.params, "integrationId"));
      ...
      res.json(await this.jiraIntegrations.saveConnection(user.userId, input, routeParam(req.params, "integrationId")));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly testJiraConnection: RequestHandler = async (req, res) => {
    try {
      const user = currentUser(req);
      const client = await getClient(this.jiraIntegrations, user.userId, routeParam(req.params, "integrationId"));
      await client.testConnection();
      res.json({ ok: true });
    } catch (error) {
      res.status(502).json({ error: error instanceof Error ? error.message : "Unable to reach Jira" });
  ...
  public readonly runJiraIntake: RequestHandler = async (req, res, next) => {
    try {
      ...
      logJiraIntakeRequest("received", {
        ...
        userId: user?.userId,
        authenticated: Boolean(user),
        integrationId: routeParam(req.params, "integrationId"),
  ...
  public readonly saveJiraBoard: RequestHandler = async (req, res, next) => {
    try {
      ...
      res.json(await this.jiraIntegrations.saveBoard(user.userId, routeParam(req.params, "integrationId"), input));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly listJiraBoardStatuses: RequestHandler = async (req, res) => {
    try {
      const user = currentUser(req);
      const boardId = Number(routeParam(req.params, "boardId"));
  ...
  public readonly saveJiraReadyStatus: RequestHandler = async (req, res, next) => {
    try {
      const user = currentUser(req);
      const settings = await this.jiraIntegrations.getSettingsForUser(user.userId, routeParam(req.params, "integrationId"));
      ...
      res.json(await this.jiraIntegrations.saveReadyStatus(user.userId, routeParam(req.params, "integrationId"), input));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly saveJiraWorkflow: RequestHandler = async (req, res, next) => {
    try {
      ...
      res.json(await this.jiraIntegrations.saveWorkflow(user.userId, routeParam(req.params, "integrationId"), input));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly setJiraEnabled: RequestHandler = async (req, res, next) => {
    try {
      ...
      const saved = await this.jiraIntegrations.setEnabled(user.userId, routeParam(req.params, "integrationId"), enabled);
  ...
  public readonly sampleReadyJiraIssue: RequestHandler = async (req, res) => {
    try {
      const user = currentUser(req);
      const credentials = await this.jiraIntegrations.getCredentials(user.userId, routeParam(req.params, "integrationId"));
  ...
  public readonly listJiraIssueTransitions: RequestHandler = async (req, res) => {
    try {
      const user = currentUser(req);
      const client = await getClient(this.jiraIntegrations, user.userId, routeParam(req.params, "integrationId"));
      res.json({ transitions: await client.getTransitions(routeParam(req.params, "issueKey")) });
    } catch (error) {
  ...
  public readonly saveJiraProcessedStatus: RequestHandler = async (req, res, next) => {
    try {
      ...
      res.json(await this.jiraIntegrations.saveProcessedStatus(user.userId, routeParam(req.params, "integrationId"), input));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly saveJiraSettings: RequestHandler = async (req, res, next) => {
    try {
      ...
      await this.jiraIntegrations.saveConnection(user.userId, connectionInput, routeParam(req.params, "integrationId"));
      res.json(await this.jiraIntegrations.saveProcessedStatus(user.userId, routeParam(req.params, "integrationId"), parseProcessedStatusInput(req.body)));
    } catch (error) {
      next(error);
    }
  };
  ...
  public readonly deleteJiraIntegration: RequestHandler = async (req, res, next) => {
    try {
      const user = currentUser(req);
      const deleted = await this.jiraIntegrations.delete(user.userId, routeParam(req.params, "integrationId"));
