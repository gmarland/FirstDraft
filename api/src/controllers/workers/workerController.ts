...
export class WorkerController {
  ...
  public readonly getWorkerState: RequestHandler = async (req, res, next) => {
    try {
      const user = req.user as User;
      const client = await this.store.getWorkerForUser(user.userId, routeParam(req.params, "workerId"));
      if (!client) {
  ...
  public readonly updateWorker: RequestHandler = async (req, res, next) => {
    try {
      ...
      const client = await this.store.setWorkerEnabledForUser(user.userId, routeParam(req.params, "workerId"), enabled);
      if (!client) {
        return res.status(404).json({ error: "worker is not registered" });
      }
  ...
  public readonly getWorkerCommand: RequestHandler = async (req, res, next) => {
    try {
      const command = await this.getVisibleCommand(req.user as User, routeParam(req.params, "workerId"), routeParam(req.params, "transactionId"));
  ...
  public readonly cancelWorkerCommand: RequestHandler = async (req, res, next) => {
    try {
      ...
      const cancelled = await this.store.cancelWorkerCommand({
        ...
        workerId: routeParam(req.params, "workerId"),
        reason: readCancelReason(req.body)
      });

      await (this.dispatcher.dispatchQueuedCommands?.(routeParam(req.params, "workerId")) ?? Promise.resolve());
      res.json(cancelled);
    } catch (error) {
      next(error);
    }
